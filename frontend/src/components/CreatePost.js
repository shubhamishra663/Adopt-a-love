...
export default function CreatePost() {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("text", postText);  // 'text' is required for the post content
    formData.append("email", userData?.user?.email || navUserData?.email);  // Assuming you're using email for user reference
    if (selectedImage) {
      formData.append("image", selectedImage);  // Optional image field
    }
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-200 p-4">
      <div className="bg-white w-full max-w-lg rounded-lg shadow-lg overflow-hidden max-h-screen">
        ...
        <form className="p-4 space-y-6 flex flex-col h-full max-h-[calc(100vh-64px)]" onSubmit={handleSubmit}>
          {/* Scrollable Section */}
